...
class SearchForm(FlaskForm):
	...
	parkCode = StringField('Park Code(s)', validators=[Length(min=4, max=10, message='Park codes can only be between %(min)d and %(max)d characters long.')], description="Codes specific to each park")

	stateCode = StringField('State Code(s)', validators=[Length(min=2, max=2, message='State codes can only be %(min)d characters.')], description="2-letter codes IDing each US State.")
	...
	start = IntegerField('Start Position', validators=[NumberRange(0, message="Start value must be above 0")])
